...
const Cart = () => {
  ...
  const amountWithoutAdo = cart?.cartItems?.reduce(
    (acc, item) => acc + item.quantity * item.price,
    0
  );

  const adoAmount = (amountWithoutAdo * 0.15).toFixed(2);

  const totalAmount = (Number(amountWithoutAdo) + Number(adoAmount)).toFixed(2);
      ...
      <section className="py-5 sm:py-7 bg-blue-100">
        <div className="container max-w-screen-xl mx-auto px-4">
          <h2 className="text-3xl font-semibold mb-2">
            {cart?.cartItems?.length || 0} Termék van a kosárban
          </h2>
        </div>
      </section>
                ...
                <article className="border border-gray-200 bg-white shadow-sm rounded mb-5 p-3 lg:p-5">
                  <ul className="mb-5">
                    <li className="flex justify-between text-gray-600  mb-1">
                      <span>Adó nélküli összeg</span>
                      <span>€{amountWithoutAdo}</span>
                    </li>
                    <li className="flex justify-between text-gray-600  mb-1">
